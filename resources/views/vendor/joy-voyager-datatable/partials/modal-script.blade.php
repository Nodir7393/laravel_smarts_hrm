    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="{{ $dataIcon }}"></i> {{ __('joy-voyager-datatable::generic.quick_add_new').' '.$displayName }}</h4>
            </div>
...
<script>
    var quickCreateAction = `{{ $route }}`;
    var quickUpdateFormAction = `{{ route('voyager.'.$dataSlug.'.quick-store') }}`;
    ...
    function deleteHandler(tag, isMulti) {
        return function() {
            ...
            if(confirm(`{{ __('voyager::generic.are_you_sure_delete') }} ${params.filename}`)) {
                $.post('{{ route('voyager.'.$dataSlug.'.media.remove') }}', params, function (response) {
                    if ( response
                        && response.data
